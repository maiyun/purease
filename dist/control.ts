...
/** --- Cascader 选中项变更事件 --- */
export interface ICascaderChangedEvent {
    'detail': {
        /** --- 选中值数组 --- */
        'value': string[];
        /** --- 选中标签数组 --- */
...
list['pe-cascader'] = {
    ...
    'data': function() {
        return {
            'value': [] as string[],
            'searchValue': '',
            'expandedLevels': [] as ICascaderOption[][],
    ...
    'methods': {
        ...
        selectOption: async function(
            this: ICascaderVue, level: number, index: number, option: ICascaderOption
        ) {
            if (option.disabled) {
                return;
            }
            ...
            // --- lazy 模式且 leaf 为 false 时，加载子节点 ---
                ...
                const event: ICascaderChangedEvent = {
                    'detail': {
                        'value': pathValues,
                        'labels': pathLabels
                    }
        ...
        hoverOption: function(this: ICascaderVue, level: number, index: number, option: ICascaderOption) {
        ...
        selectSearchResult: function(this: ICascaderVue, result: { 'path': ICascaderOption[]; 'pathValues': string[]; 'pathLabels': string[]; }) {
    ...
    'computed': {
        ...
        searchResults: function(this: ICascaderVue): Array<{ 'path': ICascaderOption[]; 'pathValues': string[]; 'pathLabels': string[]; }> {
            if (!this.searchValue) {
                return [];
            }
            const results: Array<{ 'path': ICascaderOption[]; 'pathValues': string[]; 'pathLabels': string[]; }> = [];
            const searchLower = this.searchValue.toLowerCase();
            // --- 递归搜索所有叶子节点 ---
            const searchInOptions = (
                options: ICascaderOption[],
                path: ICascaderOption[], pathValues: string[], pathLabels: string[]
            ): void => {
